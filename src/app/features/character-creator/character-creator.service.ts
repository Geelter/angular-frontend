...
export class CharacterCreatorService {
  ...
  private characterArchetypes = new Dictionary<ArchetypeModel>();

  private attributesConfig: AttributesConfigModel;

  private characterAttributes: AttributeModel[] = [];
  ...
  private async fetchCharacterArchetypes(): Promise<
    Dictionary<ArchetypeModel>
  > {
    ...
    if (characterArchetypes && !error) {
      const archetypes = characterArchetypes as ArchetypeModel[];
  ...
  getCharacterArchetypes(): Promise<Dictionary<ArchetypeModel>> {
    if (this.characterArchetypesCached()) {
      return new Promise<Dictionary<ArchetypeModel>>(resolve => {
  ...
  private async fetchAttributesConfig(): Promise<AttributesConfigModel> {
    ...
    if (character_creator_config && !error) {
      const config = character_creator_config[0] as AttributesConfigModel;
  ...
  getAttributesConfig(): Promise<AttributesConfigModel> {
    if (this.attributesConfigCached()) {
      return new Promise<AttributesConfigModel>(resolve => {
  ...
  private async fetchAttributes(): Promise<AttributeModel[]> {
    const attributesConfig: AttributesConfigModel =
      await this.getAttributesConfig();
    ...
    if (characterAttributes && !error) {
      const attributes = characterAttributes as AttributeModel[];
  ...
  getAttributes(): Promise<AttributeModel[]> {
    if (this.attributesCached()) {
      return new Promise<AttributeModel[]>(resolve => {
